...
def link_parameter_with_processor_group(processor_group_name, parameter_context):
    ...
    """
    # Get the processor group details
    pg_id = get_processor_group_info(processor_group_name)  # remove if possible
    ...
    parameter_link_body = {"revision": {"version":pg_id['revision']['version'], "lastModifier": "Python"}, "component": {"id": pg_id['component']['id'], "parameterContext":{"id": parameter_context['id']}}}
    
    link_parameter_res = rq.put(f"{prop.NIFI_IP}:{prop.NIFI_PORT}/nifi-api/process-groups/{pg_id['component']['id']}", json=parameter_link_body, headers=header)
    if link_parameter_res.status_code == 200:
        logging.info("Successfully linked parameter context with processor group")
    else:
    ...
    for i in controller_details['controllerServices']:
        if i['component']['state'] == 'DISABLED':
            ...
            if controller_service_enable_res.status_code == 200:
                ...
                logging.info(f"Controller Name = {controller_service_enable_res.json()['component']['name']}")
                logging.info(f"Controller State = {controller_service_enable_res.json()['component']['state']}")
...
if __name__ == "__main__":
    ...
    for param_name, filename in params.items():
        if param_name == parameter_context_name:
            print('param_name=',param_name,' ','filename=',filename)
            update_nifi_params.nifi_params_config(param_name, filename)
    ...
    print(link_parameter_with_processor_group(processor_group_name, parameter_context_name))
